...
try:
    while True:
        ...
        if week >= 5:  # for saturday, sunday
            monday = datetime(
                now.year, now.month, now.day, 9, 0, tzinfo=timezone("Asia/Seoul")
            )
        ...
        elif now.hour >= 21:  # after 9pm, rest until next morning 9am
            nextMorning = datetime(
                now.year, now.month, now.day, now.hour, 0, tzinfo=timezone("Asia/Seoul")
            )
            nextMorning += timedelta(hours=now.hour - 9)
            diff_secs = (nextMorning - now).seconds
            print(f"Nighttime...resting until next KST 9am: {diff_secs//60} minutes")
            time.sleep(diff_secs)
